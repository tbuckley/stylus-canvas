...
import { LitElement, html } from 'lit-element'; // ROTATION_EPSILON is the amount to rotate the canvas when
...
function getTransform({
  ...
  const translateTransform = getTranslateTransform({
    width,
    height
  }, rotation);
...
export default class StylusCanvas extends LitElement {
  ...
  render() {
    ...
    const transform = getTransform({ ...elDims,
      rotation,
      disableLowLatency
    });
  ...
  getContext(contextId, optionalContextAttributes) {
    const contextAttributes = optionalContextAttributes || {}; // Check that parameters will allow for low-latency
