...
function getCanvasWidthHeight(dimensions, rotation) {
...
export default class StylusCanvas extends HTMLElement {
  constructor() {
    super();
  ...
  _handleRotation() {
    if (this.rotation === window.screen.orientation.angle) {
      return;
    }
  ...
  updateTransform() {
    ...
    const translateTransform = `translate(${shiftX}px, ${shiftY}px)`;
  ...
  setLowLatency(enabled) {
    if (this.lowLatencyEnabled === enabled) {
      return;
    }

    this.lowLatencyEnabled = enabled;
    this.updateTransform();
  }
